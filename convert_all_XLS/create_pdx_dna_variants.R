...
TABLE_NAME = "pdx_dna_variants"

# read in metadata
filename <- dir(".",pattern = glob2rx("*_pdx_dna_variants.xlsx"))
if(length(filename) != 1) stop("too few or too many _pdx_dna_variants.xlsx sheets in dropbox")
meta <- read_excel(paste0("./",filename),sheet="Header_Data")

df <- read_excel(paste0("./",filename),sheet="pdx_dna_variants_cat",
...
df_subset$pdx_id <- stringr::str_sub(df_subset$pdx_id,1,10)
warning("Discarded route, passage info from pdx_dna_variants:pdx_id. Go back and include in table and metadata (columns).")
